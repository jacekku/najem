...
public class WorkspaceHeaderInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            subject = subjectOf();
        } catch (AccessDeniedException unresolvable) {
            // No token and no configured operator. The deployment reached here only by setting
            // najem.security.permit-all explicitly, which cannot be acquired by omission — so the
            // permissive path is one somebody chose, not one this code chose for them.
            return true;
        }
    ...
    private UUID subjectOf() {
        Jwt jwt = jwt();
        if (jwt != null) {
            return currentUser.subject(jwt);
        }
