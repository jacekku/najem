...
**The dependency list was circulated for the owners to correct before being written into a test** (najem-build seq 88). Two corrections came out of reading the real records rather than the domain docs, both recorded here because the wrong versions were in this plan:

- **`TenancyActivated(workspaceId, tenancyId, activatedOn)` carries no `unitId` and no `startDate`** — the earlier text below claimed both. The unit↔tenancy link lives on the `Unit` stream instead, in `TenancyPeriodRegistered(workspaceId, unitId, tenancyId, start, end)`, which is a better source anyway: it carries both dates, `TenancyPeriodReleased` gives the cancellation, and Task 5's occupancy spans need no cross-stream join.
- **No accounting event carries `workspaceId`** (`ChargePosted`, `PaymentIngested`, `PaymentAllocated`, `ChargeDeactivated`, `CreditNoteIssued`). `TenancyLedger` is recoverable — its `stream_id` is the tenancyId, whose workspace PM's `Tenancy` stream gives. **`Payment` is not**: placing a `PaymentIngested` requires `PaymentAllocated.chargeId` → `ChargePosted.tenancyId` → PM's tenancy → workspace, three hops to answer "whose data is this?" on the system's hard tenancy boundary. Asked accounting to add the leading field (their own internal events, no CCR); until then Tasks 4–6 derive it, and the derivation is deleted when the field lands.

- [ ] **Step 1:** Write a test enumerating every `(event_type, required field)` pair Reporting depends on — the list is in seq 88, e.g. `TenancyReserved` → `unitId`, `startDate`, `legalForm`, `monthly.total`; `PaymentAllocated` → `paymentId`, `chargeId`, `amount`. For each, drive the *owning module's own service* to emit one real event into a Testcontainers Postgres, then assert the field is present and non-null in the stored payload. Driving the real service (not a hand-written fixture) is the whole point: a hand-built JSON blob would keep passing after PM renamed the field.
- [ ] **Step 2:** Run it — it must pass on the current main, or the dependency list is already wrong.
- [ ] **Step 3:** Add a failure message naming the owning agent and the event: *"`Unit`/`TenancyPeriodRegistered` no longer carries `unitId` — Reporting's unit timeline depends on it; talk to najem-pm before changing this."* A tripwire nobody can interpret is a tripwire nobody fixes.
- [ ] **Step 4: Commit.**
