...
>
> **STATUS: PLAN ONLY — the Phase 2 code gate is CLOSED** (coordinator, najem-build seq 56 pt 3). Nothing here may be built until the gate opens *and* Decision 1 below is ruled on. Written now because Reporting's central architectural question is cheapest to settle while nobody has written a projection yet.
...
## Decision 1 — THE question this plan exists to settle: how does Reporting see other modules' facts?
...
### Task 3: Event contract tripwire

**Files:** `EventContractTest.java` (test-only task — no production code)

This is Decision 1's mitigation and the reason it is a task rather than a footnote.
...
## Open questions for the coordinator (none blocking, since the code gate is closed)

1. **Decision 1** — direct `events` read vs. integration-events-only. The one that must be settled before Task 2. My recommendation and its honest cost are above.
2. **Decision 2** — confirm arrears colours stay Accounting's and amend the roadmap's Phase 2 line. Needs @najem-accounting's agreement too.
3. **Task 3's test-scope dependency** on pm/accounting — acceptable, or fall back to golden JSON?
4. **Who owns Reporting?** I wrote this because I was idle with Contacts complete (seq 56 pt 3), not to claim the module. Happy to build it, hand it over, or have it split — say which.
5. **Build time.** @najem-fakebank's seq 57 datum: the full build went from ~25s to 21m47s when the Keycloak suite landed, and the rebase discipline has each of us running it two or three times per merge. If Reporting adds Testcontainers suites on top of that, the merge cycle gets painful. Their suggestion — tag the heavy suite, exclude it from default `build`, always-on in CI — is worth ruling on before Phase 2 adds five more projection suites.
